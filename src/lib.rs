...
//
// v0.3 — Code review fixes:
//   #1 Anchor growth capped at 2×⌈log₂(n)⌉ — prevents O(n²) on random data
//   #2 Binary searches fused in update() — single partition_point call
//   #3 PlacementSort boundary-only repair — O(B) not O(L²) on violation
//   #4 Confidence gating in PlacementSort — model earns its overhead
//   #5 Steeper accuracy-driven entropy decay — decay tied to prediction quality
//   #6 Fixed-count disorder sampling (64 pairs) — O(1) instead of O(n)
//   #7 Stack-allocated histogram for small segments — no Vec alloc per call
...
const GALLOP_WIN: usize = 7;
const DISORDER_SAMPLES: usize = 64; // fix #6: fixed sample count, O(1)

// ── Anchor cap helper — fix #1 ──────────────────────────────────────────────
fn max_anchors(n: usize) -> usize {
    2 * ((n as f64).log2().ceil() as usize).max(2)
}
...
pub struct DistributionModel {
    ...
    running_surprise: f64, // fix #5: track rolling prediction accuracy
}
...
impl DistributionModel {
    ...
    pub fn update(&mut self, value: f64, actual_position: usize, n: usize) {
        ...
        // fix #2: compute insertion position once, reuse for both
        // surprise calculation and potential anchor insertion.
        // Previously: predict() ran partition_point, then update() ran it again.
        let insert_pos = self.anchors.partition_point(|&a| a <= value);

        // Interpolate predicted position using the same insert_pos
        let predicted_pos = if self.anchors.is_empty() {
            0.5
        } else {
            let k = self.anchors.len();
            let min = self.estimated_min;
            ...
            if (max - min).abs() < f64::EPSILON {
                ...
                let lo = self.anchors[insert_pos - 1];
                let hi = self.anchors[insert_pos];
                let frac = if (hi - lo).abs() < f64::EPSILON { 0.5 }
                else { (value - lo) / (hi - lo) };
                let lo_p = (insert_pos - 1) as f64 / (k - 1) as f64;
                let hi_p = insert_pos as f64 / (k - 1) as f64;
                lo_p + frac * (hi_p - lo_p)
            }
        };
        ...
        // fix #5: accuracy-driven entropy decay — steeper when model is accurate,
        // gentler when predictions are poor. Replaces fixed 0.01 schedule.
        let obs_ratio = self.observations as f64 / n as f64;
        self.running_surprise = self.running_surprise * 0.95 + surprise * 0.05;
        let accuracy = 1.0 - self.running_surprise.min(1.0);
        self.entropy = (self.entropy * (1.0 - accuracy * obs_ratio * 0.1)).max(0.0);

        // fix #1: cap anchor count to prevent O(n²) insertion on random data.
        // Once cap reached, average-in the new value at the nearest anchor
        // instead of inserting.
        if surprise > 0.1 {
            let cap = max_anchors(n);
            if self.anchors.len() < cap {
                self.anchors.insert(insert_pos, value);
            } else {
                let nearest = insert_pos.min(self.anchors.len() - 1);
                self.anchors[nearest] = (self.anchors[nearest] + value) / 2.0;
            }
        }
    }

    // fix #7: stack-allocated histogram for small segments.
    // For n <= 64, use a fixed-size array on the stack instead of allocating
    // a Vec. For larger segments, fall back to the heap allocation.
    pub fn local_entropy(values: &[f64]) -> f64 {
        ...
        // Stack path for small segments (covers most Trivial-routed segments)
        if buckets <= 16 {
            ...
            return h / (buckets as f64).log2();
        }

        // Heap path for large segments
...
impl<T: PartialOrd + Into<f64> + Copy> VisionSort<T> {
    ...
    fn phase3_disorder_map(&self, data: &[T], segments: Vec<Segment>) -> Vec<Segment> {
        ...
        // Fast path: single segment covering the whole array.
        // This is the Reversed / All Same case — Phase 2 already handled it.
        // Skip all scoring, route directly to Verify (O(n) scan, does nothing
        // if already sorted). Eliminates 1,700μs Phase 3 overhead on Reversed.
        if segments.len() == 1 {
            let s = &segments[0];
            if s.start == 0 && s.end == n {
                let mut seg = segments.into_iter().next().unwrap();
                seg.disorder = 0.0;
                seg.entropy = 0.0;
                seg.route = SortRoute::Verify;
                return vec![seg];
            }
        }

        // fix #7: avoid Vec<f64> allocation for entropy by computing
        // min/max/histogram in a single pass using inline float conversion.
        let mut scored: Vec<Segment> = segments.into_iter().map(|mut seg| {
            ...
            // Compute entropy without intermediate Vec<f64> for small segments
            let seg_len = slice.len();
            let entropy = if seg_len <= 256 {
                // Stack buffer path — compute min/max then histogram inline
                let mut mn = f64::MAX;
                let mut mx = f64::MIN;
                for x in slice { let v: f64 = (*x).into(); if v < mn {mn=v;} if v > mx {mx=v;} }
                if (mx - mn).abs() < f64::EPSILON {
                    0.0
                } else {
                    let buckets = (seg_len as f64).sqrt() as usize + 1;
                    let mut counts = [0u16; 18]; // covers sqrt(256)+1 = 17 buckets
                    let nb = buckets.min(18);
                    for x in slice {
                        let v: f64 = (*x).into();
                        let idx = ((v - mn) / (mx - mn) * (nb - 1) as f64) as usize;
                        counts[idx.min(nb - 1)] += 1;
                    }
                    let mut h = 0.0f64;
                    for &c in &counts[..nb] {
                        if c > 0 { let p = c as f64 / seg_len as f64; h -= p * p.log2(); }
                    }
                    h / (nb as f64).log2()
                }
            } else {
                let floats: Vec<f64> = slice.iter().map(|x| (*x).into()).collect();
                DistributionModel::local_entropy(&floats)
            };
        ...
        for seg in scored {
            ...
            match seg.route {
                ...
                SortRoute::PlacementSort => {
                    // fix #4: gate placement on confidence. High-confidence
                    // elements get direct bucket assignment. Low-confidence
                    // elements fall back to comparison-based placement within
                    // their bucket — the model earns its overhead.
                    let len = seg.end - seg.start;
                    let b = ((len as f64).sqrt().ceil() as usize).max(1);
                    let mut buckets: Vec<Vec<T>> = vec![Vec::new(); b];
                    let mut fallback: Vec<T> = Vec::new();
                    let values: Vec<T> = data[seg.start..seg.end].to_vec();

                    for &val in &values {
                        let (predicted_pos, confidence) = model.predict(val.into());
                        if confidence >= 0.5 {
                            // Confident — assign to predicted bucket
                            let bucket_idx = ((predicted_pos * (b - 1) as f64).round() as usize).min(b - 1);
                            buckets[bucket_idx].push(val);
                        } else {
                            // Low confidence — collect for comparison-based placement
                            fallback.push(val);
                        }
                    }

                    // Sort confident placements within each bucket
                    for bucket in buckets.iter_mut() {
                        Self::insertion_sort(bucket);
                    }

                    // Sort fallback elements and merge into appropriate buckets
                    if !fallback.is_empty() {
                        Self::introsort(&mut fallback, depth_limit);
                        // Distribute sorted fallback into correct buckets
                        for &val in &fallback {
                            let (predicted_pos, _) = model.predict(val.into());
                            let bucket_idx = ((predicted_pos * (b - 1) as f64).round() as usize).min(b - 1);
                            // Insert maintaining sort order within bucket
                            let pos = buckets[bucket_idx].partition_point(|x| *x <= val);
                            buckets[bucket_idx].insert(pos, val);
                        }
                    }

                    // Write sorted buckets back and update model
                    ...
                    // fix #3: boundary-only repair instead of full re-sort.
                    // Only check and fix the B-1 cross-bucket boundaries,
                    // costing O(B) instead of O(L²) for a full insertion sort.
                    let bucket_count = b;
                    let mut write_pos = seg.start;
                    for bi in 0..bucket_count {
                        write_pos += buckets[bi].len();
                        if bi + 1 < bucket_count && !buckets[bi].is_empty() && !buckets[bi + 1].is_empty() {
                            // Check boundary: last element of bucket[bi] vs first of bucket[bi+1]
                            if data[write_pos - 1] > data[write_pos] {
                                // Repair: insertion-sort a small window spanning the boundary
                                let lo = write_pos.saturating_sub(buckets[bi].len().min(8));
                                let hi = (write_pos + buckets[bi + 1].len().min(8)).min(seg.end);
                                Self::insertion_sort(&mut data[lo..hi]);
                            }
                        }
                    }

                    // Final linear scan to catch any remaining violations
                    // (should be rare after boundary repair)
                    let slice = &mut data[seg.start..seg.end];
                    let violations = (1..slice.len()).any(|i| slice[i - 1] > slice[i]);
                    if violations { Self::insertion_sort(slice); }
                }
    ...
    fn gallop_merge(data: &mut [T], lo: usize, mid: usize, hi: usize) {
        ...
        while l < left_len && r < hi {
            if left[l] <= data[r] {
                ...
                data[out] = left[l]; l += 1; out += 1;
                if left_wins >= GALLOP_WIN {
                    let mut step = 1usize; let mut count = 0usize;
                    while l + step <= left_len && left[l + step - 1] <= data[r] { count = step; step *= 2; }
                    let upper = (l + step).min(left_len);
                    let take = left[l..upper].partition_point(|x| x <= &data[r]).max(count).min(left_len - l);
                    for k in 0..take { data[out + k] = left[l + k]; }
                    l += take; out += take; left_wins = 0;
                }
            } else {
                right_wins += 1; left_wins = 0;
                data[out] = data[r]; r += 1; out += 1;
                if right_wins >= GALLOP_WIN {
                    let mut step = 1usize; let mut count = 0usize;
                    while r + step <= hi && data[r + step - 1] < left[l] { count = step; step *= 2; }
                    let upper = (r + step).min(hi);
                    let take = data[r..upper].partition_point(|x| x < &left[l]).max(count).min(hi - r);
                    let src = r;
                    for k in 0..take { data[out + k] = data[src + k]; }
                    r += take; out += take; right_wins = 0;
                }
            }
        }
        while l < left_len { data[out] = left[l]; l += 1; out += 1; }
    }
    ...
    fn median_of_three(data: &[T]) -> usize {
        let mid = data.len() / 2; let last = data.len() - 1;
    ...
    // fix #6: fixed-count disorder sampling — DISORDER_SAMPLES (64) random pairs
    // gives statistically equivalent estimates in O(1) regardless of segment size.
    // Previous approach: (√n choose 2) ≈ n/2 pairs — O(n) per segment.
    fn estimate_disorder(data: &[T]) -> f64 {
        let n = data.len();
        if n <= 1 { return 0.0; }

        // Use a simple LCG to pick pairs deterministically (no allocation, no RNG dep)
        let mut state: u64 = 0xdeadbeef_cafebabe ^ n as u64;
        let mut inversions = 0usize;
        let pairs = DISORDER_SAMPLES.min(n * (n - 1) / 2);
        ...
        for _ in 0..pairs {
            ...
            if a != b && data[a.min(b)] > data[a.max(b)] {
                inversions += 1;
            }
        }
...
mod tests {
    ...
    fn std_sorted(mut data: Vec<f64>) -> Vec<f64> {
        data.sort_by(|a, b| a.partial_cmp(b).unwrap()); data
    }
    ...
    #[test] fn test_route_decision() {
        ...
        assert_eq!(SortRoute::decide(0.1,0.1,100),SortRoute::Trivial);
    }

    #[test]
    fn stress_random_1k() {
        let mut d: Vec<f64>=lcg(1000,42).iter().map(|&x|x as f64).collect();
        let e=std_sorted(d.clone()); vision_sort(&mut d); assert_eq!(d,e,"random 1k");
    }
    #[test]
    fn stress_random_10k() {
        let mut d: Vec<f64>=lcg(10_000,99).iter().map(|&x|x as f64).collect();
    ...
    fn stress_nearly_sorted_10k() {
        ...
        let e=std_sorted(d.clone()); vision_sort(&mut d); assert_eq!(d,e);
    }
    #[test]
    ...
    fn stress_clustered_10k() {
        ...
        let seq=lcg(10_000,13);
        let mut d: Vec<f64>=seq.iter().enumerate().map(|(i,&x)|bands[i%5]+(x%10) as f64).collect();
        let e=std_sorted(d.clone()); vision_sort(&mut d); assert_eq!(d,e);
    }
    #[test]
    fn stress_duplicates_10k() {
        let seq=lcg(10_000,55);
        let mut d: Vec<f64>=seq.iter().map(|&x|(x%100) as f64).collect();
        let e=std_sorted(d.clone()); vision_sort(&mut d); assert_eq!(d,e);
    }
    #[test]
    fn cost_curve_entropy_decreases() {
        let data: Vec<f64>=lcg(1000,42).iter().map(|&x|x as f64).collect();
        let mut sorter=VisionSort::new();
        sorter.model=sorter.phase1_glance(&data);
        let initial=sorter.model.entropy;
        let n=data.len();
        let samples=lcg(100,7);
        for (i,&s) in samples.iter().enumerate(){
            let val=data[s%n] as f64;
            sorter.model.update(val,i,n);
        }
        assert!(sorter.model.entropy<=initial,
            "entropy should decrease: {}->{}",initial,sorter.model.entropy);
    }
    #[test]
    fn anchor_count_bounded() {
        // fix #1 regression test: anchor count must not exceed 2*log2(n)
        let mut model = DistributionModel::empty();
        let n = 1000;
        let cap = max_anchors(n);
        let seq = lcg(n, 7);
        for (i, &v) in seq.iter().enumerate() {
            model.update(v as f64, i, n);
        }
        assert!(model.anchors.len() <= cap,
            "anchors {} exceeded cap {}", model.anchors.len(), cap);
    }

    fn lcg(n: usize, seed: u64) -> Vec<usize> {
        let mut s=seed;
        (0..n).map(|_|{s=s.wrapping_mul(6364136223846793005).wrapping_add(1442695040888963407);(s>>33) as usize}).collect()
    }
}
